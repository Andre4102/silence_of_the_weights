...
pruning_strategies=("same_channel" "per_head" "entire_head")
...
for pruning in "${pruning_strategies[@]}"; do
  for threshold in "${threshold_strategies[@]}"; do
    for importance in "${importance_strategies[@]}"; do
      ...
      sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=ast${short_pruning}${short_threshold}${short_importance}${ds}_10
#SBATCH --output=pruning_ast_logs/%x.out
#SBATCH --error=pruning_ast_logs/%x.err
#SBATCH --ntasks=1
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --mem=40G
...
#SBATCH --partition=A40,A100,L40S
