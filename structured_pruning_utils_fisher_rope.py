...
from custom_attention import MultiheadAttention, Attention, LlamaAttention, CLIPAttention, BertAttention, WhisperAttention, CustomViTAttention, CustomDinov2Attention
...
def apply_structured_pruning(model, pruning_plan: Dict, remove_empty_layers: bool = True):
    ...
    for layer_idx, layer in enumerate(layers):
        ...
        # Update dimensions
        if isinstance(sa, CustomViTAttention):
            sa.attention.num_heads_q = len(kept_heads_q)
            sa.attention.num_heads_k = len(kept_heads_k)
            sa.attention.num_heads_v= len(kept_heads_o)
            sa.attention.num_heads_out = len(kept_heads_o)

            sa.output.num_heads_v = len(kept_heads_o)

            sa.attention.head_dim_q = len(kept_channels_q)
            sa.attention.head_dim_k = len(kept_channels_k)
            sa.attention.head_dim_v = len(kept_channels_v)
            sa.attention.head_dim_out = len(kept_channels_v)

            sa.output.head_dim_v = len(kept_channels_v)

            # print('changing the scaling')
            sa.attention.scaling = sa.attention.head_dim_q**-0.5
            
            sa.attention.q_out_dim = sa.attention.head_dim_q*sa.attention.num_heads_q
            sa.attention.k_out_dim = sa.attention.head_dim_k*sa.attention.num_heads_k
            sa.attention.v_out_dim = sa.attention.head_dim_v*sa.attention.num_heads_v
        ...
        elif isinstance(sa, CustomDinov2Attention):
            sa.attention.num_heads_q = len(kept_heads_q)
            sa.attention.num_heads_k = len(kept_heads_k)
            sa.attention.num_heads_v = len(kept_heads_o)
            sa.attention.num_heads_out = len(kept_heads_o)
            sa.output.num_heads_out = len(kept_heads_o)
            
            sa.attention.head_dim_q = len(kept_channels_q)
            sa.attention.head_dim_k = len(kept_channels_k)
            sa.attention.head_dim_v = len(kept_channels_v)
            sa.attention.head_dim_out = len(kept_channels_o)
            sa.output.head_dim_out = len(kept_channels_o)

            sa.attention.all_head_size_q = sa.attention.num_heads_q * sa.attention.head_dim_q
            sa.attention.all_head_size_k = sa.attention.num_heads_k * sa.attention.head_dim_k
            sa.attention.all_head_size_v = sa.attention.num_heads_v * sa.attention.head_dim_v
            sa.attention.all_head_size_out = sa.attention.num_heads_out * sa.attention.head_dim_out
            sa.output.all_head_size_out = sa.output.num_heads_out * sa.output.head_dim_out

        elif hasattr(sa, 'num_heads_q'):
        ...
        if isinstance(sa, (MultiheadAttention)):
            ...
            # Extract slices
            q_w = new_weight[:sa.self.all_head_size_q].contiguous()
            q_b = new_bias[:sa.self.all_head_size_q].contiguous()
            k_w = new_weight[sa.self.all_head_size_q:sa.self.all_head_size_q+sa.self.all_head_size_k].contiguous()
            k_b = new_bias[sa.self.all_head_size_q:sa.self.all_head_size_q+sa.self.all_head_size_k].contiguous()
            v_w = new_weight[sa.self.all_head_size_q+sa.self.all_head_size_k:].contiguous()
            v_b = new_bias[sa.self.all_head_size_q+sa.self.all_head_size_k:].contiguous()
            o_b = sa.output.dense.bias.data.clone() if sa.output.dense.bias is not None else None
            ...
            sa.self.query.weight.data = q_w
            sa.self.query.bias.data = q_b
            
            sa.self.key = nn.Linear(embed_dim, sa.self.all_head_size_k, device=device)
            sa.self.key.weight.data = k_w
            sa.self.key.bias.data = k_b
            
            sa.self.value = nn.Linear(embed_dim, sa.self.all_head_size_v, device=device)
            sa.self.value.weight.data = v_w
            sa.self.value.bias.data = v_b
            
            sa.output.dense = nn.Linear(sa.self.all_head_size_v, embed_dim, device=device)
            sa.output.dense.weight.data = new_o_weight.contiguous()
            if o_b is not None:
                sa.output.dense.bias.data = o_b
                del o_b
            
            del q_w, q_b, k_w, k_b, v_w, v_b
            
            if hasattr(sa.self, 'distance_embedding'):
                sa.self.distance_embedding = torch.nn.Embedding(2 * sa.self.max_position_embeddings - 1, sa.self.head_dim_q)
            ...
            o_b = sa.o_proj.bias.data.clone() if sa.o_proj.bias is not None else None
            ...
            del sa.o_proj
            torch.cuda.empty_cache() if torch.cuda.is_available() else None
            sa.scaling = sa.head_dim_q**-0.5
            
            # Create new modules
            sa.q_proj = nn.Linear(embed_dim, sa.q_out_dim, device=device, bias=has_bias)
            sa.q_proj.weight.data = q_w
            
            sa.k_proj = nn.Linear(embed_dim, sa.k_out_dim, device=device, bias=has_bias)
            ...
            sa.o_proj = nn.Linear(sa.v_out_dim, embed_dim, device=device, bias=(o_b is not None))
            sa.o_proj.weight.data = new_o_weight.contiguous()
            if o_b is not None:
                sa.o_proj.bias.data = o_b
                del o_b
            
            del q_w, k_w, v_w
            #rope is handles above
                 
        elif isinstance(sa, CLIPAttention):
            device = next(sa.parameters()).device
            ...
            # Extract slices
            q_w = new_weight[:sa.attention.all_head_size_q].contiguous()
            k_w = new_weight[sa.attention.all_head_size_q:sa.attention.all_head_size_q+sa.attention.all_head_size_k].contiguous()
            v_w = new_weight[sa.attention.all_head_size_q+sa.attention.all_head_size_k:].contiguous()
            
            has_bias = new_bias is not None
            if has_bias:
                q_b = new_bias[:sa.attention.all_head_size_q].contiguous()
                k_b = new_bias[sa.attention.all_head_size_q:sa.attention.all_head_size_q+sa.attention.all_head_size_k].contiguous()
                v_b = new_bias[sa.attention.all_head_size_q+sa.attention.all_head_size_k:].contiguous()
            
            o_b = sa.output.dense.bias.data.clone() if sa.output.dense.bias is not None else None
            
            # Delete old modules
            del sa.attention.query, sa.attention.key, sa.attention.value, sa.output.dense
            torch.cuda.empty_cache() if torch.cuda.is_available() else None
            
            # Create new modules
            sa.attention.query = nn.Linear(embed_dim, sa.attention.all_head_size_q, device=device, bias=has_bias)
            sa.attention.query.weight.data = q_w
            if has_bias:
                sa.attention.query.bias.data = q_b
            
            sa.attention.key = nn.Linear(embed_dim, sa.attention.all_head_size_k, device=device, bias=has_bias)
            sa.attention.key.weight.data = k_w
            if has_bias:
                sa.attention.key.bias.data = k_b
            
            sa.attention.value = nn.Linear(embed_dim, sa.attention.all_head_size_v, device=device, bias=has_bias)
            sa.attention.value.weight.data = v_w
            if has_bias:
                sa.attention.value.bias.data = v_b
            
            sa.output.dense = nn.Linear(sa.output.all_head_size_out, embed_dim, device=device, bias=(o_b is not None))
            sa.output.dense.weight.data = new_o_weight.contiguous()
            if o_b is not None:
                sa.output.dense.bias.data = o_b
        ...
        elif isinstance(sa, CustomViTAttention):
            device = next(sa.parameters()).device
            
            # Extract slices
            q_w = new_weight[:sa.attention.q_out_dim].contiguous()
            k_w = new_weight[sa.attention.q_out_dim:sa.attention.q_out_dim+sa.attention.k_out_dim].contiguous()
            v_w = new_weight[sa.attention.q_out_dim+sa.attention.k_out_dim:].contiguous()
            
            has_bias = new_bias is not None
            if has_bias:
                q_b = new_bias[:sa.attention.q_out_dim].contiguous()
                k_b = new_bias[sa.attention.q_out_dim:sa.attention.q_out_dim+sa.attention.k_out_dim].contiguous()
                v_b = new_bias[sa.attention.q_out_dim+sa.attention.k_out_dim:].contiguous()
            ...
            # Create new modules
            sa.attention.query = nn.Linear(embed_dim, sa.attention.q_out_dim, device=device, bias=has_bias)
            sa.attention.query.weight.data = q_w
            if has_bias:
                sa.attention.query.bias.data = q_b
            
            sa.attention.key = nn.Linear(embed_dim, sa.attention.k_out_dim, device=device, bias=has_bias)
            sa.attention.key.weight.data = k_w
            if has_bias:
                sa.attention.key.bias.data = k_b
            
            sa.attention.value = nn.Linear(embed_dim, sa.attention.v_out_dim, device=device, bias=has_bias)
            sa.attention.value.weight.data = v_w
            if has_bias:
                sa.attention.value.bias.data = v_b
            
            sa.output.dense = nn.Linear(sa.attention.v_out_dim, embed_dim, device=device, bias=(o_b is not None))
            sa.output.dense.weight.data = new_o_weight.contiguous()
            if o_b is not None:
                sa.output.dense.bias.data = o_b
